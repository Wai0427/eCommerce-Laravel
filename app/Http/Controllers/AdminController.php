...
class AdminController extends Controller
{
    ...
    public function sendPasswordResetLink(Request $request){
        ...
        if( $oldToken ){
            ...
            //If there is no token found, add new token
    ...
    public function resetPassword(Request $request, $token = null){
        ...
        if( $check_token ){
            ...
            if( $diffMins > constDefaults::tokenExpiredMinutes ){
                ...
                //If $diffMins not more than 15min or constDefaults::tokenExpiredMinutes, display reset-password page
                return view('back.pages.admin.auth.reset-password')->with(['token'=>$token]);
            }
        }else {
            //If this token not assist, redirect the user to admin-forgot-password with fail message
    ...
    public function resetPasswordHandler(Request $request){
        //First to validate the form
        $request->validate([
            'new_password'=>'required|min:5|max:45|required_with:new_password_confirmation|same:new_password_confirmation','new_password_confirmation'=>'required'
        ]);
    ...
    //Send email to notify admin address
    ...
    $mailConfig = array(
        ...
        //If email sent, redirect user to admin.login page with success message
    ...
    public function changeProfilePicture(Request $request){
        ...
        if($upload){
            //If upload picture success, delete old picture
            if( $old_picture != null && File::exists(public_path($path.$old_picture)) ){
                File::delete(public_path($path.$old_picture));
            }
            //Update admin profile name in data base
    ...
    public function insertProduct(){
        ...
        return view('back.insert_product', compact('admin'));
    }
